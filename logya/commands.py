...
def _is_stale(rel_file: Path, search_index: dict, static_path: Path) -> bool:
...
def clean(dir_site: str, verbose: bool, **_kwargs) -> None:
    ...
    search_index = L.collection_index | L.doc_index
    ...
    for r, dirs, files in walk(L.paths.public, topdown=False):
        ...
        # Remove stale files
        for f in files:
            rel_file = root.relative_to(L.paths.public) / f
            if _is_stale(rel_file, search_index, L.paths.static):
        ...
        for d in dirs:
            ...
            rel_dir = root.relative_to(L.paths.public) / d
